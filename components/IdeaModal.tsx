...
export default function IdeaModal({ idea, isOpen, onClose, onDelete, onDeleteIdea, onCommentAdded, apiPrefix = '/api', userName }: IdeaModalProps) {
    const { author, displayText } = parseAuthor(idea.text);
    const [copied, setCopied] = useState(false);
    const [isClosing, setIsClosing] = useState(false);
    const [commentText, setCommentText] = useState('');
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [showColectivizarMenu, setShowColectivizarMenu] = useState(false);
    const [colectivizado, setColectivizado] = useState(false);
    ...
    const handleColectivizar = async (mode: 'anon' | 'user') => {
        const publicText = mode === 'user' && userName ? `[${userName}]: ${displayText}` : displayText;
        setShowColectivizarMenu(false);
        await fetch('/api/ideas', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: publicText }),
        });
        setColectivizado(true);
    };
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {onDeleteIdea && (
                            <button
                                ...
                                className="flex items-center justify-center w-10 h-10 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"
                                title="Borrar idea"
                            >
                        ...
                        {apiPrefix === '/api/privado' && (
                            <div className="relative">
                                {colectivizado ? (
                                    ...
                                    <button
                                        onClick={() => setShowColectivizarMenu(v => !v)}
                                        className="w-10 h-10 flex items-center justify-center rounded-xl text-gray-400 hover:text-[#C5A47E] hover:bg-[#C5A47E]/10 transition-all"
                                        title="Colectivizar idea"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
                                    </button>
                                )}
                                {showColectivizarMenu && !colectivizado && (
                                    <div className="absolute bottom-full right-0 mb-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-10 min-w-[140px]">
                                        <button
                                            className="w-full text-left px-4 py-2.5 text-xs text-gray-600 hover:bg-gray-50 hover:text-[#C5A47E] transition-colors"
                                            onClick={() => handleColectivizar('anon')}
                                        >
                                            Anónimo
                                        </button>
                                        <button
                                            ...
                                            onClick={() => handleColectivizar('user')}
                                        >
                                            Con usuario
                                        </button>
                                    </div>
